...
def main(X_scaled, y, k=10, ):
    ...
    # best_knn_DB, best_params_DB, accuracy_DB = fzKNN_knn_grid_search(X_train, y_train, X_test, y_test, param_grid)
     
    # print(f"\nBest Parameters (DB_KNN): {best_params_DB}")
    # print(f"Accuracy on 70-25 split: {accuracy_DB:.2f}")

    print("\nTuning CB_KNN using the 70-25 split...")
    best_knn_CB, best_params_CB, accuracy_CB = cbKNN_knn_grid_search(X_train, y_train, X_test, y_test, param_grid)
     
    print(f"\nBest Parameters (CB_KNN): {best_params_CB}")
    print(f"Accuracy on 70-25 split: {accuracy_CB:.2f}")

    # use the bets paprameter found for the dataset and initlise the models with those hyper parameters
    ...
    # knn_DB=fzKNN(n=best_params_DB['n_neighbors'],
    #                        weights=best_params_DB['weights'],
    #                        metric=best_params_DB['metric'])
    
    

    knn_sklearn = KNeighborsClassifier(n_neighbors=best_params_CB['n_neighbors'],
                                       weights=best_params_CB['weights'],
                                       metric=best_params_CB['metric'])
    
    cb_KNN=cbKNN(n=best_params_CB['n_neighbors'],
                                       weights=best_params_CB['weights'],
                                       metric=best_params_CB['metric'])
    

    models={'knn_custom':knn_custom,'knn_CB':cb_KNN,'knn_sklearn':knn_sklearn}
